...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const connectWs = React.useCallback(() => {
    ...
    try {
      ...
      ws.onerror = () => {
        // Fehler → schließen; onclose triggert Reconnect
        try {
          ws.close();
        } catch {}
      };

      // Wir werten Messages hier nicht aus; Live-Status reicht für Navbar.
      ws.onmessage = () => {};
    } catch {
      setLive(false);
      reconnectRef.current.tries += 1;
  ...
  // Theme-Toggle aus globaler Funktion (von /app/providers.tsx registriert)
    ...
    <Box sx={{ display: "flex", minHeight: "100dvh" }}>
      ...
      <Box
        ...
        sx={{
          ...
          width: "100%",
          mx: "auto",
          ...theme.mixins.toolbar, // verhindert Überlappung mit der fixen AppBar
          pt: { xs: 2, md: 10 },
        }}
      >
        {children}
