...
export function requireRole(req: NextRequest, allowed: string[]): AuthzResult {
    ...
    try {
        ...
        const roles: string[] = [
            ...
            ...Object.values(decoded?.resource_access ?? {}).flatMap((x: any) => x?.roles ?? []),
        ].map((r: string) => r.toLowerCase());

        const ok = allowed.some((r) => roles.includes(r.toLowerCase()));
        return ok ? { ok: true, role: roles.find((r) => allowed.map((a) => a.toLowerCase()).includes(r)) } : { ok: false, reason: "forbidden" };
    } catch {
