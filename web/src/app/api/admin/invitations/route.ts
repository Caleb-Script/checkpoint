...
export async function GET(req: NextRequest) {
  ...
  const mapped = list.map((i) => {
    const type = i.invitedByInvitationId
      ? ("plusone" as const)
      : ("main" as const);
    const invitedByName = i.invitedBy
      ? `${i.invitedBy.guestProfile?.firstName ?? ""} ${i.invitedBy.guestProfile?.lastName ?? ""}`.trim() ||
        null
      : null;
    ...
    return {
      ...
      guest: {
        ...
        name:
          `${i.guestProfile?.firstName ?? ""} ${i.guestProfile?.lastName ?? ""}`.trim() ||
          null,
      },
      ticket: i.ticket
        ? {
