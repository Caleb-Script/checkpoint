...
export async function POST(req: NextRequest) {
    ...
    if (!shareCodes.length && !ids.length) {
        return NextResponse.json({ ok: false, error: "provide shareCodes or invitationIds" }, { status: 400 });
    }
    ...
    if (!invitations.length) return NextResponse.json({ ok: false, error: "no invitations found" }, { status: 404 });
    ...
    for (const inv of invitations) {
        ...
        let ticket = await prisma.ticket.findUnique({ where: { invitationId: inv.id } });
        if (!ticket) {
