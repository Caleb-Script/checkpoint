...
import { createUser, findUserByUsernameOrEmail, setUserTempPassword } from "@/lib/keycloakAdmin";
...
export async function POST(req: NextRequest) {
    ...
    if (!invitationId && !shareCode) {
        return NextResponse.json({ ok: false, error: "invitationId or shareCode required" }, { status: 400 });
    }
    ...
    if (inv.status !== "ACCEPTED") {
        return NextResponse.json({ ok: false, error: "invitation-not-accepted" }, { status: 409 });
    }

    // Ticket anlegen (falls fehlt)
    const ticket = inv.ticket ?? await prisma.ticket.create({
    ...
    const displayName = `${inv.guestProfile?.firstName ?? ""} ${inv.guestProfile?.lastName ?? ""}`.trim();
    const user = await prisma.user.upsert({
        where: { keycloakId: kcId! },
        update: { email: email || null, name: displayName || null },
        create: { keycloakId: kcId!, email: email || null, name: displayName || null, roles: ["guest"] },
    });
    if (!inv.guestProfile?.userId) {
        await prisma.guestProfile.update({ where: { id: inv.guestProfileId }, data: { userId: user.id } });
    }

    // ===== Links bauen =====
    const base = process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000";
    // Signierter Token für Ticket-/QR-Endpunkte (Schutz vor Copy-Share)
    const claim = signToken({ kind: "ticket", invitationId: inv.id, ticketId: ticket.id }, 60 * 60 * 24 * 7); // 7 Tage
