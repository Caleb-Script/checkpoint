...
export async function GET() {
  try {
    const tokRes = await fetch(
      `${KC_BASE}/realms/${encodeURIComponent(KC_REALM)}/protocol/openid-connect/token`,
      {
        method: "POST",
    ...
    if (!tokRes.ok)
      return NextResponse.json(
        { ok: false, where: "token", status: tokRes.status, tok },
        { status: 500 },
      );

    // kleine Probe: Darf /users lesen?
    const r = await fetch(
      `${KC_BASE}/realms/${encodeURIComponent(KC_REALM)}/users?max=1`,
      {
    ...
    const payload = JSON.parse(
      Buffer.from(tok.access_token.split(".")[1], "base64").toString("utf8"),
    );
