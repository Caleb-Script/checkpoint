...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!eventName && !eventId) {
            return NextResponse.json({ ok: false, error: "eventName oder eventId erforderlich" }, { status: 400 });
        }
        if (!email && !phone) {
        ...
        invitation = await prisma.invitation.update({
            ...
            data: {
                ...
                ...(rsvpChoice === "NO" ? { status: "DECLINED", approved: false, approvedAt: null, approvedById: null } : {}),
            },
        });
