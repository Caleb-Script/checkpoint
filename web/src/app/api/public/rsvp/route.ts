...
export async function POST(req: NextRequest) {
    ...
    if (!code || !choice) return NextResponse.json({ ok: false, error: "code & choice required" }, { status: 400 });
    ...
    const status = choice === "YES" ? InvitationStatus.ACCEPTED : InvitationStatus.DECLINED;
    ...
    return NextResponse.json({ ok: true, invitation: { id: updated.id, status: updated.status, rsvpChoice: updated.rsvpChoice } });
}
