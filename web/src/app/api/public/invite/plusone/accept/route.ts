...
export async function POST(req: NextRequest) {
  ...
  const toAccept = children
    .filter(
      (c) =>
        c.invitedByInvitationId ===
          parent.id /* egal, ob bisher PENDING oder DECLINED */ && !c.ticket,
    )
    .map((c) => c.id);
  if (!toAccept.length)
    return NextResponse.json(
      { ok: false, error: "nothing-to-accept" },
      { status: 400 },
    );
  ...
  await prisma.invitation.updateMany({
    ...
    data: { status: "ACCEPTED", rsvpChoice: "YES", rsvpAt: now },
  });
