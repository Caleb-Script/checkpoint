...
export async function DELETE(req: NextRequest) {
    ...
    const deletable = children.filter(c => c.invitedByInvitationId === parent.id && !c.ticket).map(c => c.id);
    if (!deletable.length) return NextResponse.json({ ok: false, error: "nothing-deletable" }, { status: 400 });
