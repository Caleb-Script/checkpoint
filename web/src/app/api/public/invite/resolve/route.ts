...
export async function GET(req: NextRequest) {
    ...
    const regToken = isMain ? signToken({ kind: "invite-reg", invitationId: inv.id }, 60 * 60 * 4) : null; // 4h
    ...
    return NextResponse.json({
        ...
        invitation: {
            ...
            event: { id: inv.eventId, name: inv.event.name, startsAt: inv.event.startsAt, endsAt: inv.event.endsAt },
            guest: {
            ...
            invitedBy: inv.invitedBy
                ? { name: `${inv.invitedBy.guestProfile?.firstName ?? ""} ${inv.invitedBy.guestProfile?.lastName ?? ""}`.trim() || null }
                : null,
