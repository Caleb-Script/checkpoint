...
export async function GET(req: NextRequest) {
    ...
    const inv = await prisma.invitation.findUnique({
        ...
        include: {
            guestProfile: { select: { firstName: true, lastName: true, primaryEmail: true } },
    ...
    const link = inv.shareCode ? `${base}/invite?code=${encodeURIComponent(inv.shareCode)}` : null;
...
export async function POST(req: NextRequest) {
    ...
    const maxInvitees = typeof body?.maxInvitees === "number" ? Math.max(0, Math.floor(body.maxInvitees)) : undefined;
