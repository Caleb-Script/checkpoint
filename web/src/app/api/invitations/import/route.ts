  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!r.email && !r.phone) {
        results.push({
          rowIndex: i,
          error: "Weder email noch phone vorhanden – übersprungen",
        });
        continue;
      }
      ...
      try {
        const link = personalLink(
          opts.webUrlBase || "",
          r,
          opts.eventName || event.name,
        );
        const appPart =
          opts.includeAppStore && opts.appStoreUrl
            ? `App (optional): ${opts.appStoreUrl}`
            : "";
        const seat = seatLabel(r);

        const message = renderMessage(
          opts.messageTemplate || DEFAULT_TEMPLATE,
          {
        ...
        wa = {
          mode: "link",
          status: "skipped",
          reason: `Fehler beim Linkbau: ${String(e?.message || e)}`,
        };
      }
