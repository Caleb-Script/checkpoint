...
const QR_SECRET =
  process.env.QR_JWT_SECRET || process.env.JWT_SECRET || "dev-secret";
const KC_CLIENT_ID =
...
function extractRolesFromJwt(accessToken: string): string[] {
  try {
    ...
    const clientRoles: string[] =
      json?.resource_access?.[KC_CLIENT_ID]?.roles || [];
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!qrToken) {
      return NextResponse.json(
        { error: "Kein QR-Token übermittelt" },
        { status: 400 },
      );
    }
    ...
    if (ticket.revoked) {
      return NextResponse.json(
        { error: "Ticket wurde widerrufen" },
        { status: 403 },
      );
    }
    if (ticket.eventId !== decoded.eventId) {
      return NextResponse.json(
        { error: "Ticket gehört nicht zu diesem Event" },
        { status: 403 },
      );
    }
    ...
    if (decoded.direction === "INSIDE") {
      if (ticket.currentState === "INSIDE") {
        return NextResponse.json(
          { error: "Person ist bereits drinnen" },
          { status: 409 },
        );
      }
      newState = "INSIDE";
      ...
      if (ticket.currentState === "OUTSIDE") {
        return NextResponse.json(
          { error: "Person ist bereits draußen" },
          { status: 409 },
        );
      }
