...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const ids = Array.isArray(body.invitationIds)
      ? body.invitationIds.filter(Boolean)
      : [];
    if (ids.length === 0) {
      return NextResponse.json(
        { error: "invitationIds ist leer" },
        { status: 400 },
      );
    }
    ...
    if (!base) {
      return NextResponse.json(
        { error: "webBaseUrl fehlt (und Origin nicht verfügbar)" },
        { status: 400 },
      );
    }
    base = base.replace(/\/+$/, "");
    ...
    for (const inv of invitations) {
      ...
      if (!inv.approved) {
        results.push({
          invitationId: inv.id,
          ok: false,
          error: "Nicht freigegeben (approved=false)",
        });
        continue;
      }
      ...
      if (
        seatOverride &&
        (seatOverride.section || seatOverride.row || seatOverride.number)
      ) {
      ...
      // Nachricht
      const guestName =
        [inv.guestProfile?.firstName, inv.guestProfile?.lastName]
          .filter(Boolean)
          .join(" ") || "Gast";
