...
function ensureEnvOrError(): { baseUrl: string; realm: string; clientId: string } | { error: string } {
...
function profileFromJwt(jwt: JwtPayload | null): Record<string, any> {
    if (!jwt) return {};
    const { sub, email, name, given_name, family_name, preferred_username, ...rest } = jwt;
    return {
        sub,
        email,
        name,
...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!accessToken) {
            return NextResponse.json({ authenticated: false, reason: "no_access_token" }, { status: 401 });
        }
        ...
        if ("error" in cfg) {
            ...
            return NextResponse.json(
                { authenticated: true, profile, roles, tokenExpiresAt, source: "jwt", warn: `Config: ${cfg.error}` },
                { status: 200 }
            );
        }
        ...
        if (resp.ok) {
            ...
            return NextResponse.json(
                { authenticated: true, profile, roles, tokenExpiresAt, source: "userinfo" },
                { status: 200 }
            );
        }
        ...
        return NextResponse.json({ authenticated: false, error: "Serverfehler" }, { status: 500 });
    }
}