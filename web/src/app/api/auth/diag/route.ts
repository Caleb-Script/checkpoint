...
export async function GET(req: NextRequest) {
    try {
        ...
        const clientId = getEnv("KC_CLIENT_ID") || getEnv("NEXT_PUBLIC_KC_CLIENT_ID") || "checkpoint-guest";
        ...
        // 1) ENV Check
        if (!envOk) {
            return NextResponse.json({ ok: false, step: "env", results }, { status: 500 });
        }
        ...
        try {
            ...
            results.openid = { status: r.status, ok: r.ok, url: openIdConfigUrl, raw: json ?? t };
            if (!r.ok) {
                return NextResponse.json({ ok: false, step: "openid", results }, { status: 500 });
            }
        } catch (e: any) {
            results.openid = { error: String(e) };
            return NextResponse.json({ ok: false, step: "openid_fetch", results }, { status: 500 });
        }

        // 3) /userinfo testen (nur wenn access token vorhanden)
        if (!accessToken) {
            results.userinfo = { note: "kein kc_access_token Cookie vorhanden" };
            return NextResponse.json({ ok: false, step: "no_access_token", results }, { status: 401 });
        }
        ...
        try {
            ...
            let json: any = null;
            try { json = JSON.parse(t); } catch { }
            results.userinfo = { status: r.status, ok: r.ok, url: userinfoUrl, raw: json ?? t };
            if (!r.ok) {
                return NextResponse.json({ ok: false, step: "userinfo", results }, { status: 500 });
            }
        } catch (e: any) {
            results.userinfo = { error: String(e) };
            return NextResponse.json({ ok: false, step: "userinfo_fetch", results }, { status: 500 });
        }
