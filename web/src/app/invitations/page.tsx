...
import {
  Box, Button, Card, CardContent, CardHeader, Stack, TextField, Typography,
  MenuItem, Select, FormControl, InputLabel, Table, TableBody, TableCell, TableHead, TableRow, Checkbox,
...
export default function InvitationsPage() {
  ...
  const [csvText, setCsvText] = React.useState<string>("email,phone,firstName,lastName,maxInvites\nalice@example.com,,Alice,Example,4");
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (json.ok) setEvents(json.events.map((e: any) => ({ id: e.id, name: e.name })));
    })();
  }, []);
  ...
  async function refreshList() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/invitations?eventId=${encodeURIComponent(eventId)}`);
      const json = await res.json();
  ...
  React.useEffect(() => { void refreshList(); }, [eventId]);

  function toggle(id: string) {
    setSelected((s) => ({ ...s, [id]: !s[id] }));
  }

  function buildWaUrl(link: string, name?: string | null, phone?: string | null) {
  ...
  async function importData() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/invitations/import?eventId=${encodeURIComponent(eventId)}`, {
        method: "POST",
        body: fd,
      });
      const json = await res.json();
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1200, mx: "auto" }}>
      <Typography variant="h5" sx={{ mb: 2, fontWeight: 700 }}>Einladungen – Import & Versand</Typography>
      ...
      <Card sx={{ mb: 3 }}>
        ...
        <CardContent>
          <Stack spacing={2}>
            ...
            <FormControlLabel
              control={<Switch checked={onlyMain} onChange={(_, c) => setOnlyMain(c)} />}
      ...
      <Card sx={{ mb: 3 }}>
        <CardHeader title="2) Importieren" subheader="CSV/Excel hochladen und/oder CSV-Text einfügen (Spalten: email, phone, firstName, lastName, maxInvites)" />
        <CardContent>
          <Stack spacing={2}>
            <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems="center">
              ...
              <Typography variant="body2" color="text.secondary">
                {file ? `Ausgewählt: ${file.name}` : "Keine Datei ausgewählt (optional)"}
              </Typography>
              {file && (
            ...
            <Stack direction="row" spacing={2}>
              <Button variant="contained" onClick={importData} disabled={!eventId || loading}>
                Import starten
              </Button>
              <Button variant="outlined" onClick={refreshList} disabled={!eventId || loading}>
                Aktualisieren
              </Button>
            </Stack>
          </Stack>
      ...
      {/* 3) Liste + WhatsApp */}
      <Card>
        <CardHeader title="3) Einladungen & WhatsApp" subheader="Typ beachten: Haupt-Einladung vs. Plus-One" />
        <CardContent>
          <Table size="small">
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {visibleRows.map((r) => (
                <TableRow key={r.id} hover>
                  ...
                  <TableCell>
                    {r.type === "main" ? "Haupt" : `Plus-One${r.invitedByName ? ` von ${r.invitedByName}` : ""}`}
                  </TableCell>
                  ...
                  <TableCell sx={{ maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {r.inviteLink ? (
                      <Stack direction="row" spacing={1} alignItems="center">
                        <a href={r.inviteLink} target="_blank" rel="noreferrer">{r.inviteLink}</a>
                        <Tooltip title="Link kopieren">
                          <IconButton size="small" onClick={() => navigator.clipboard.writeText(r.inviteLink!)}>
                  ...
                  <TableCell>
                    {r.inviteLink && (
                      <Button size="small" variant="outlined" href={buildWaUrl(r.inviteLink, r.guest?.name, r.guest?.phone)} target="_blank">
                        WhatsApp
                      </Button>
                    )}
                  </TableCell>
