...
export async function pushToCheckpoint(
...
): Promise<{ ok: boolean; seatCount: number; pushedTo?: string; warnings?: string[] }> {
    ...
    if (!res.ok) {
        ...
        return { ok: false, seatCount: seatMap.seats.length, warnings, pushedTo: options.webhookUrl };
    }
    return { ok: true, seatCount: seatMap.seats.length, pushedTo: options.webhookUrl, warnings };
}
