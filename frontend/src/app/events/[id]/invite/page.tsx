...
export default function EventAdminInvitePage() {
  ...
  // Einladungen (globaler Query → per eventId gefiltert)
  const {
    data: invData,
    loading: invLoading,
    error: invError,
  ...
  // zuletzt erzeugte Links (z. B. nach Bulk)
  ...
  async function createManyFromCsv() {
    ...
    for (const r of rows) {
      try {
        ...
        if (id) {
          const url = rsvpLinkForInvitationId(id);
          links.push(url);
        }
        ok++;
      } catch {
  ...
  async function approve(id: string) {
    setErr(null);
    await updateInvitation({ variables: { id, approved: true } });
    await refetchInvs();
  }
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, maxWidth: 1000, mx: 'auto' }}>
      ...
      <Card sx={{ mb: 2 }}>
        <CardHeader
          ...
          subheader="CSV/Text oder Datei – optional mit Spalte maxInvitees (nur für Kontingent). Der RSVP-Link wird aus der Invitation-ID gebaut."
        />
        <CardContent>
                        ...
                        <TableCell align="right">
                          ...
                          >
                            <IconButton
                              ...
                              disabled={updating || inv.approved}
                            >
