...
import {
  Box, Button, Card, CardContent, TextField, Stack, Typography, Alert,
...
export default function LoginPage() {
  ...
  const [doLogin, { loading }] = useMutation(LOGIN, { client, fetchPolicy: "no-cache" });
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!data?.login) return setError(data?.login?.error || "Anmeldung fehlgeschlagen.");
  ...
  return (
    <Box sx={{ minHeight: "100dvh", display: "grid", placeItems: "center", p: 2 }}>
      <Card sx={{ width: "100%", maxWidth: 420, borderRadius: 3, boxShadow: 6 }}>
        <CardContent sx={{ p: 4 }}>
          <Typography variant="h5" fontWeight={800} gutterBottom>Anmelden</Typography>
          ...
          <form onSubmit={onSubmit}>
            <Stack spacing={2}>
              ...
              />
              <Button type="submit" size="large" variant="contained" startIcon={<LoginRoundedIcon/>} disabled={loading}>
                {loading ? (<><CircularProgress size={18} sx={{ mr: 1 }}/> Anmelden…</>) : "Anmelden"}
              </Button>
            </Stack>
          </form>
        </CardContent>
